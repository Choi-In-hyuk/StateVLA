...
class BesoResNetEncoder(nn.Module):
    """ResNet18 encoder for single camera images."""
    ...
    def __init__(
        ...
        # Substitute BatchNorm with GroupNorm for diffusion compatibility
        replace_submodules(
...
class MultiImageResNetEncoder(nn.Module):
    """Multi-camera ResNet encoder combining multiple ResNet18s."""
    ...
    def forward(self, obs_dict: Dict[str, torch.Tensor]) -> torch.Tensor:
        ...
        Returns:
            torch.Tensor: Features from all cameras [batch_size, num_cameras, latent_dim]
        """
        features = []
