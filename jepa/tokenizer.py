...
class ImageTokenizer(nn.Module):
    ...
    def __init__(self, image_size: int = 224, patch_size: int = 16, in_channels: int = 3, embed_dim: int = 256):
        super().__init__()
...
class LanguageTokenizer(nn.Module):
    ...
    def __init__(self, lang_emb_dim: int = 512, embed_dim: int = 256, num_tokens: int = 1):
        super().__init__()
...
class PretrainedLanguageTokenizer(nn.Module):
    ...
    def __init__(self, model_name: str = "ViT-B/32", embed_dim: int = 256, freeze_backbone: bool = True, device: str = 'cuda'):
        super().__init__()
        self.embed_dim = embed_dim
        self.device = device
        self.num_tokens = 1
...
class RobotStateTokenizer(nn.Module):
    ...
    def __init__(self, robot_state_dim: int = 8, embed_dim: int = 256, num_tokens: int = 1):
        super().__init__()
...
class MultiModalTokenizer(nn.Module):
    ...
    def __init__(
        ...
        if use_pretrained_vision:
            self._init_pretrained_vision(camera_names, vision_model_name, embed_dim, freeze_vision, device)
        else:
            self.num_patches_per_image = (image_size // patch_size) ** 2
            self.image_tokenizers = nn.ModuleDict({
                name: ImageTokenizer(image_size=image_size, patch_size=patch_size, embed_dim=embed_dim)
        ...
        if use_pretrained_language:
            self.language_tokenizer = PretrainedLanguageTokenizer(
                model_name=language_model_name, embed_dim=embed_dim, freeze_backbone=freeze_language, device=device
            )
        else:
            self.language_tokenizer = LanguageTokenizer(lang_emb_dim=lang_emb_dim, embed_dim=embed_dim, num_tokens=1)
    ...
    def _init_pretrained_vision(self, camera_names: List[str], model_name: str, embed_dim: int, freeze: bool, device: str):
        ...
        self.image_pos_embeds = nn.ParameterDict({
            name: nn.Parameter(torch.zeros(1, self.num_patches_per_image, embed_dim)) for name in camera_names
        })

        for name in camera_names:
            nn.init.trunc_normal_(self.image_pos_embeds[name], std=0.02)
        print(f"SigLIP loaded. Hidden: {self.hidden_size}, Patches per img: {self.num_patches_per_image}")

    def _encode_image_pretrained(self, img: torch.Tensor, camera_name: str) -> torch.Tensor:
        if img.shape[-1] != self.vision_image_size:
            img = F.interpolate(img, size=(self.vision_image_size, self.vision_image_size), mode='bilinear', align_corners=False)
    ...
    def forward(self, obs_dict: Dict[str, torch.Tensor]) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        lang_tokens = lang_tokens + self.modality_embed(torch.tensor(self.MODALITY_LANGUAGE, device=device))
        all_tokens.append(lang_tokens)
        all_modality_ids.append(torch.full((B, self.num_lang_tokens), self.MODALITY_LANGUAGE, device=device, dtype=torch.long))
        ...
        robot_tokens = robot_tokens + self.modality_embed(torch.tensor(self.MODALITY_ROBOT_STATE, device=device))
        all_tokens.append(robot_tokens)
        all_modality_ids.append(torch.full((B, self.num_robot_tokens), self.MODALITY_ROBOT_STATE, device=device, dtype=torch.long))

        # [3. Vision Patches] Processes images with Lang & Robot context in hidden states
        for i, name in enumerate(self.camera_names):
            img_key = 'agentview_image' if name == 'agentview' else 'eye_in_hand_image' if name == 'eye_in_hand' else f'{name}_image'
            modality_id = self.MODALITY_AGENTVIEW if name == 'agentview' else self.MODALITY_WRIST if name == 'eye_in_hand' else i
            ...
            img_tokens = img_tokens + self.modality_embed(torch.tensor(modality_id, device=device))
            all_tokens.append(img_tokens)
            all_modality_ids.append(torch.full((B, self.num_patches_per_image), modality_id, device=device, dtype=torch.long))
